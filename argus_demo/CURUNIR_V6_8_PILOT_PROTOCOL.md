...
Use one new campaign root. Never reuse, edit, or delete a mission root after
...
The primary operator and approver must be different real people. Automated
...
person after signing out, using the separately constrained public actor.
...
export CAMPAIGN_ROOT=/home/moloch/Saulot/curunir_v68_runs/V68_TERMINAL_001
$V68_PYTHON -m tools.curunir_v68 prepare-all --campaign-root "$CAMPAIGN_ROOT"
```

`prepare-all` performs live public acquisition for M1. If an external source
...
7. Create a dossier. Every factual sentence must cite visible exact basis;
   inference must be `EXPLICITLY_INFERENTIAL` with a note; gaps must be
   `UNRESOLVED`. Submit it for review.
...
2. Record two competing hypotheses for the affected facility. Treat the
   Croatian page as a dependent derivative, not independent corroboration.
...
5. Reassess the hypotheses. Produce and submit a report that distinguishes
   current observation, superseded historical state, inference, and unknowns.
6. End the primary session. The distinct approver reviews exact evidence,
   approves the validated report, and records an `APPROVER` start/end pair.
...
enumerable. Do not obtain the restricted id from the cleared session merely to
probe it. Record the public access-check end.
...
may remain `PASS_WITH_ACCEPTED_BASELINE_RESIDUALS`; the terminal validator
preserves that qualified outcome and rejects any new nonpass or clean-checkout
failure.
