...
from .access import AccessContext, context_from_record, marking_from_record
from .canonical import utc_now
from .explain import explain, explain_markdown
from .pipelines import build_connector, executor_with_registered_connectors
...
def main(argv: list[str] | None = None) -> int:
    ...
    def cmd(name, **kw):
        ...
        p.add_argument("--store", required=True, help="mission data store path")
        return p

    p = cmd("init")
    p.add_argument("--store-id", required=True)
    p.add_argument("--time", default=None)
    p = cmd("register-schema")
    p.add_argument("--file", required=True)
    p.add_argument("--time", default=None)
    p = cmd("register-mapping")
    p.add_argument("--file", required=True)
    p.add_argument("--time", default=None)
    p = cmd("register-pipeline")
    p.add_argument("--file", required=True)
    p.add_argument("--time", default=None)
    p = cmd("ingest")
    ...
    p.add_argument("--source-time", default=None)
    p.add_argument("--received-time", default=None)
    p.add_argument("--marking", default=None)
    p = cmd("run-scenario")
    p.add_argument("--out", required=True)
    p = cmd("project")
    p.add_argument("--context", required=True)
    p.add_argument("--as-of-seq", type=int, default=None)
    ...
    p.add_argument("--modification", default="")
    p.add_argument("--time", default=None)
    ...
    p = cmd("replay")
    p.add_argument("--export", dest="export_dir", required=True)
    p.add_argument("--context", required=True)
    p.add_argument("--snapshot-time", required=True)
    p.add_argument("--staleness", default=None)
    ...
    if args.command == "init":
        store = MissionDataStore.create(args.store, args.store_id, now)
        _print(store.head())
        return 0
    ...
    if args.command == "import":
        ...
        _print(imported.head())
        return 0
    if args.command == "run-scenario":
        # A heavy fixture module, imported only when the command needs it.
        from .scenario.runner import run_scenario
        _print(run_scenario(Path(args.store), Path(args.out))["summary"])
        return 0
    ...
    if args.command == "register-schema":
        ...
        result = executor.run(args.pipeline, Path(args.file).read_bytes(), source_id=args.source,
                              source_time=args.source_time, received_time=args.received_time or now,
                              recorded_time=now, actor="cli", marking_override=marking)
        _print(result)
        ...
        if not chosen:
            print(f"error: workshop not registered: {args.workshop}", file=sys.stderr)
            return 2
