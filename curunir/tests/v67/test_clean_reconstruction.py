...
def test_clean_checkout_reconstruction_uses_committed_bytes_and_pinned_kernel(tmp_path):
    kernel = Path(os.environ.get("CURUNIR_ARGUS_KERNEL", PACKAGE_ROOT.parent / "kernel" / "argus"))
    ...
    assert (output / "curunir" / "curunir_workbench").is_dir()
    assert (output / "curunir" / "argus" / "prospective" / "freezing.py").is_file()
