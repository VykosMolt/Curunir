"""Reading evidence: every observation points back at the exact bytes it came
from, when something was true is kept apart from when we learned it, copies of
one source count once, and an identity match stays a proposal."""
...
def test_structured_record_keeps_fields_addressable(tmp_path):
    ...
    # An observation points at the field it came from, not just the document.
...
def test_historical_manifestation_updates_historical_state_not_today(tmp_path):
    ...
    assert claim["valid_from"] == "2008-03-01T12:00:00+00:00"  # true then
    assert claim["recorded_time"].startswith("2026-")           # learned now
...
def test_derivative_sources_share_one_dependence_family(tmp_path):
    ...
    assert claims
    claim = claims[0]
    # Two copies of one site: the second adds no support and no new version.
...
def test_independent_origins_count_separately(tmp_path):
    ...
    store = pipeline.store
    # The same words on two sites are two sources. Each site has its own claim
    # subject, so count at the source level instead.
    groups = set()
...
def test_ambiguous_identity_stays_a_reversible_proposal(tmp_path):
    ...
    # The two objects stay separate records whatever was proposed.
...
def test_out_of_order_older_evidence_does_not_displace_newer_state(tmp_path):
    ...
    # An older reading of the same source arrives after the newer one.
...
def test_reexecution_is_idempotent_and_replay_reconstructs(tmp_path):
    ...
    store = pipeline.store
    head = store.head()
    pipeline2 = make_pipeline(tmp_path, start_minute=120)  # a restart, from disk
    ...
    # The bytes travel with the export, so fields are still addressable.
...
def test_cross_scheme_identity_never_self_accepts(tmp_path):
    """An identifier match asserted by a third party stays a proposal for a
    person: nothing is accepted or clustered by machine."""
    pipeline = make_pipeline(tmp_path)
    wikidata_body = json.dumps({"entities": {"Q999001": {
        "id": "Q999001",
        "labels": {"en": {"language": "en", "value": "Vessia Steel"}},
...
def test_historical_capture_retrieved_today_does_not_displace_current_state(tmp_path):
    """How new evidence is depends on when the source said it, not when we
    fetched it: an old capture read today is still old."""
    pipeline = make_pipeline(tmp_path)
    live = plant_manifestation(
        pipeline, source_id="live-web", native_id="https://vessia.example/about",
        body=PAGE_V1, media_type="text/html",
    ...
    capture = plant_manifestation(
        ...
        retrieval_time="2026-08-17T13:05:00+00:00",  # fetched after the live page
        temporal_status="HISTORICAL",
        source_time="2008-03-01T12:00:00+00:00",
        archive_capture_time="2008-03-01T12:00:00+00:00")
...
def test_processing_failure_is_durable_and_retried(tmp_path, monkeypatch):
    """A failure leaves an open review item and is retried, never counted done."""
    pipeline = make_pipeline(tmp_path)
    plant_manifestation(pipeline, source_id="gleif", native_id="TESTLEI0000000000001",
                        body=GLEIF_RECORD_V1, media_type="application/vnd.api+json",
                        retrieval_time="2026-08-17T12:05:00+00:00")
...
def test_feed_without_channel_title_integrates_without_crash(tmp_path):
    """A subject with events but no attributes still integrates."""
    pipeline = make_pipeline(tmp_path)
...
def test_unlocatable_text_value_gets_honest_document_anchor(tmp_path):
    """A value that cannot be located anchors to the whole document and says so,
    rather than pointing at a span that does not hold it."""
    pipeline = make_pipeline(tmp_path)
...
def test_source_time_never_defaults_to_retrieval_time(tmp_path):
    """When we fetched a record never becomes when it was true."""
    pipeline = make_pipeline(tmp_path)
...
def test_two_writers_on_one_root_extend_one_chain(tmp_path):
    """Two handles on one store extend a single chain."""
    from curunir_semantic.store import SemanticStore
    pipeline = make_pipeline(tmp_path)
...
def test_wayback_enumeration_shares_the_site_family(tmp_path):
    """An archive listing counts as the archived site, in the store and in the
    planner alike."""
    from curunir_semantic.worldmodel import dependence_group_for
...
def test_anchor_position_is_the_producing_occurrence_not_the_first(tmp_path):
    """The anchor points at the text that produced the observation, even when
    the same value appears earlier in the document."""
    pipeline = make_pipeline(tmp_path)
    ...
    assert text[anchor["start"]:anchor["end"]] == anchor["exact_value"]
    # "Acme Holding" also appears in the heading; the anchor must sit in the
    # footer line that actually produced the observation.
